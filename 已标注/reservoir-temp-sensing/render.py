# -*- coding: utf-8 -*-
"""L0 intent.yaml + layout.json -> 原理图 SVG（油箱温度传感图工作副本）。

自 skill 模板 render_l0_sheet.py 复制修改,模板约定不改,增量三处:
  1. wire_taps():渲染 taps 感温/测量支路(规范 11.1)。sensor 的 measurement
     端口经 1.0 T 实线正交支路挂入 at 端口所在网络;接入点取该网络折线上
     距传感器引出点最近的投影点(超界取端点),垂直接入,不与主管线共线。
     接入点若非任何元件端口,画三通实心点。
  2. self_check():渲染完成前的结构自检(rendering-rules"结构自检")。
     part 无节点 / path 段无折线 / tap 无支路,任一缺项打印并以退出码 1
     终止,不输出成品。
  3. write_manifest():<name>_topology.md 追溯清单(节点映射/连接映射/
     测量支路/简化说明),行号取自 intent 源文本。
...
NAME = 'reservoir-temp-sensing'
INTENT_PATH = os.path.join(HERE, NAME + '.intent.yaml')
LAYOUT_PATH = os.path.join(HERE, NAME + '.layout.json')
...
SHEET_PATH = os.path.join(HERE, NAME + '.svg')
MANIFEST_PATH = os.path.join(HERE, NAME + '_topology.md')
...
def read_symbol(path):
    ...
    "0 0 218 564",温度件是 "0 26 394 210"/"0 7 132 70"。端口坐标是
    viewBox 用户坐标,须减去原点再乘缩放。
    """
    tree = ET.parse(path)
    root = tree.getroot()
...
def proj_point(a, b, p):
    """p 到正交线段 ab 的投影(线段限邻)。水平段投影 x,竖直段投影 y。"""
    if abs(a[1] - b[1]) < 0.5:                       # 水平段
        return (min(max(p[0], min(a[0], b[0])), max(a[0], b[0])), a[1])
    return (a[0], min(max(p[1], min(a[1], b[1])), max(a[1], b[1])))


def near_pt(p, q, tol=0.5):
...
class Sheet(object):
    ...
    # ---------- taps 感温/测量支路(L0 规范 11.1;本副本增量) ----------
    def wire_taps(self, path_polys):
        """渲染 taps。返回接入三通点列表(非端口接入点)。

        接入点 = at 端口所在网络(首/末点含该端口的 path 折线)上,距
        传感器引出点最近的投影点。支路自传感器 measurement 端口按锚点
        引出,垂直接入网络,接入处画三通实心点;接入点恰为某元件端口时
        由符号自带的红色端口圆标记,不另画点。
        """
        juncs = []
        for t in self.i.get('taps') or []:
            tok = t['sensor']
            sinst, spid = tok.split('.', 1) if '.' in tok else (tok, None)
            if spid is None:
                ct = self.types[self.i['parts'][sinst]]
                meas = [p['id'] for p in ct['ports'] if p['role'] == 'measurement']
                if len(meas) != 1:
                    raise ValueError('%s: measurement 端口不唯一(%s),'
                                     '须显式写 sensor 端口' % (tok, meas))
                spid = meas[0]
            ainst, apid = t['at'].split('.', 1)
            ax, ay, aa = self.abs[(sinst, spid)]
            bx, by, _ba = self.port(t['at'])
            self.port_lt[(sinst, spid)] = 'sense'

            # at 端口所在网络的折线集合
            nets = []
            for _lt, pts in path_polys:
                if near_pt(pts[0], (bx, by)) or near_pt(pts[-1], (bx, by)):
                    nets.append(pts)
            if not nets:
                raise ValueError('taps %s: %s 所在网络无已画折线,'
                                 '取压点悬空' % (tok, t['at']))
            ...
            stub = {'left': (-S, 0), 'right': (S, 0),
                    'up': (0, -S), 'down': (0, S)}[aa]
            a1 = (ax + stub[0], ay + stub[1])
            proj = [proj_point(pts[k], pts[k + 1], a1)
                    for pts in nets for k in range(len(pts) - 1)]
            tx, ty = min(proj, key=lambda p: (p[0] - a1[0]) ** 2
                         + (p[1] - a1[1]) ** 2)

            obs = self.obstacles(exclude=(sinst,))
            cands = []
            if abs(a1[0] - tx) < 0.5 or abs(a1[1] - ty) < 0.5:
                cands.append([a1, (tx, ty)])
            cands.append([a1, (tx, a1[1]), (tx, ty)])
            cands.append([a1, (a1[0], ty), (tx, ty)])
            for cx in self.L.get('vlanes', []):
                cands.append([a1, (cx, a1[1]), (cx, ty), (tx, ty)])
            best, bad = None, None
            for c in cands:
                pts = self.dedup([(ax, ay)] + c)
                if len(pts) < 2:
                    continue
                h = self.hits(pts, obs, skip_ends=True)
                ov = self.overlap(pts, self.drawn)
                txb = self.hits(pts, self.textboxes, tol=0.0)
                cr = self.crossings(pts, self.drawn)
                ln = sum(abs(pts[k + 1][0] - pts[k][0])
                         + abs(pts[k + 1][1] - pts[k][1])
                         for k in range(len(pts) - 1))
                sc = (h * 10000 + ov * 3000 + txb * 900
                      + cr * 120 + ln + len(pts) * 5)
                if bad is None or sc < bad:
                    bad, best = sc, pts
            for k in range(len(best) - 1):
            ...
            # 接入点不是任何元件端口/外部接口时画三通点
            marks = list(self.abs.values()) + [
                (e['x'], e['y'], e['anchor']) for e in self.L['externs'].values()]
            if not any(near_pt((tx, ty), (mx, my)) for mx, my, _a in marks):
                juncs.append((tx, ty))
        return juncs
    ...
    def dangling(self):
        ...
        for t in self.i.get('taps') or []:
            sinst, spid = t['sensor'].split('.', 1) if '.' in t['sensor'] \
                else (t['sensor'], None)
            if spid is None:
                ct = self.types[self.i['parts'][sinst]]
                meas = [p['id'] for p in ct['ports'] if p['role'] == 'measurement']
                if len(meas) == 1:
                    used.add((sinst, meas[0]))
            else:
                used.add((sinst, spid))
        marks, names = [], []
...
PRESSURE_CLASS = {
    ...
    'suction': 'low',      # 低于回油压力
    'case_drain': 'low',
    'sense': 'low',        # 感温/测量支路:1.0 T 实线,不属压力等级,归低
}
...
def css(T):
  ...
  line.pl-sense      { stroke-width: calc(%(lo).2f * var(--kc)); }
...
def legend(L, T):
    ...
    rows = [
        ...
        ('Sensing Branch 测量支路 (感温探头接管)              1.0 T', lo),
...
def title_block(L, intent, nnet, dnames, ntaps):
    t = L['title_block']
    ...
    row2 = ('部件 %d   |   网络 %d   |   测量支路 %d   |   未知项 %d   '
            '|   临时符号 3 (油箱 draft,温度件 provisional)   '
            '|   悬空端口 %d: %s'
            % (len(intent['parts']), nnet, ntaps, len(intent['unknown']),
...
def self_check(intent, layout, s, path_polys, tap_polys):
    ...
    for t, (_lt, pts) in zip(intent.get('taps') or [], tap_polys):
        ...
        sinst, spid = tok.split('.', 1) if '.' in tok else (tok, None)
        if spid is None:
            ct = s.types[s.i['parts'][sinst]]
            spid = [p['id'] for p in ct['ports'] if p['role'] == 'measurement'][0]
        q = s.abs[(sinst, spid)]
        if not (near_pt(pts[0], q, 1.0)):
            missing.append('tap 支路未起自 sensor 端口: %s' % tok)
    if missing:
        print('结构自检失败,扣留成品:')
        for m in missing:
            print('  -', m)
...
def intent_line_map(text):
    """parts/extern/paths 行号复用 preflight.line_map;taps 行本地补扫。"""
    lm = preflight.line_map(text)
    lm['taps'] = []
    section = None
...
def write_manifest(path, intent, layout, s, lm, path_polys, tap_polys, dnames):
    L = []
    L.append('# %s 追溯清单' % intent['system'])
    L.append('')
    L.append('来源: `%s`(L0 v%s,目录 %s,成熟度 %s)。'
             % (os.path.basename(INTENT_PATH), intent['l0_version'],
                intent['catalog'], intent['maturity']))
    L.append('图面: `%s`,布局 `%s`。' % (os.path.basename(SHEET_PATH),
                                          os.path.basename(LAYOUT_PATH)))
    L.append('')
    L.append('## 节点(part)映射')
    L.append('')
    L.append('| intent 行 | 实例 | 类型 | 图上元件 | 符号文件 |')
    L.append('|---|---|---|---|---|')
    for inst, typ in intent['parts'].items():
        nd = layout['nodes'].get(inst, {})
        L.append('| %d | %s | %s | inst-%s | %s |'
                 % (lm['parts'].get(inst, 0), inst, typ, inst,
                    os.path.basename(nd.get('symbol', '缺'))))
    for eid, etyp in intent.get('extern', {}).items():
        e = layout['externs'][eid]
        L.append('| %d | %s | extern:%s | 边界标记 (%d,%d) | — |'
                 % (lm['extern'].get(eid, 0), eid, etyp, e['x'], e['y']))
    L.append('')
    L.append('## 连接(边)映射')
    L.append('')
    ...
    for pi, p in enumerate(intent['paths']):
        ...
        for k in range(len(p) - 1):
            a, b = p[k], p[k + 1]
            qa, qb = s.port(a, 'out'), s.port(b, 'in')
    ...
    L.append('')
    L.append('1. 油箱用目录内唯一油箱类型 bootstrap_reservoir(带液位计自增压'
             '油箱,符号 draft 状态,图签栏已披露)。目录没有开放式常压油箱类型。')
    L.append('2. 目录的 bootstrap_reservoir 没有"油箱本体感温口";两只感温件'
             '的测点按 taps 规则挂在 TANK-001.return_in 所在回油网络'
             '(安装点取网络最近接入点),油箱内油温整体分布未建模'
             '(unknown: TANK-001.bulk_temp_sense_port_not_in_catalog)。')
    L.append('3. 感温件的电输出口(TT-001/TSW-001.elec_out)去向未声明,'
             '按悬空端口标红,不代为接线;冻结契约不含 signal 节,'
             '远传指示未建模(unknown: *.elec_out_destination)。')
    L.append('4. 油箱自增压气源(TANK-001.bootstrap_pressure_in)与吸油去向'
             '(TANK-001.suction_out,泵未声明)未建模,端口悬空标红。')
    L.append('5. 悬空端口 %d 个: %s。' % (len(dnames), ' '.join(dnames)))
    L.append('')
...
def main():
    ...
    if not rep['ok']:
        ...
        sys.exit(1)
    with io.open(LAYOUT_PATH, encoding='utf-8') as f:
        layout = json.load(f)
    ...
    # taps 感温支路:先记下 path 折线,taps 接入其网络,随后一并求交叉。
    path_polys = list(s.polys)
    npath_polys = len(path_polys)
    tap_junc = s.wire_taps(path_polys)
    ...
    P.append('<text class="lbl" x="%d" y="44">油箱温度传感原理图  '
             '(由 %s 生成; 油箱符号 draft, 温度件 provisional, 不可用于工程放行)</text>'
             % (40, os.path.basename(INTENT_PATH)))
    ...
    P.append('<g id="title">%s</g>' % '\n'.join(
        title_block(layout, intent, nnet, dnames, len(intent.get('taps') or []))))
    P.append('</svg>')

    # 结构自检(rendering-rules):任一缺项即退出码 1,不输出成品。
    self_check(intent, layout, s, path_polys, tap_polys)

    with io.open(SHEET_PATH, 'w', encoding='utf-8') as f:
        f.write('\n'.join(P))
    print('wrote', SHEET_PATH)
    print('nets=%d  taps=%d  segments=%d  junctions=%d  buses=%s'
          % (nnet, len(tap_polys), len(segs), len(junc),
             {k: len(v) for k, v in bus.items()}))
    for w in s.warn:
        print('WARN', w)

    write_manifest(MANIFEST_PATH, intent, layout, s,
                   intent_line_map(io.open(src, encoding='utf-8').read()),
                   path_polys, tap_polys, dnames)
