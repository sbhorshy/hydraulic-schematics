...
SHEET = os.path.join(HERE, 'reservoir-temp-sensing.svg')
LAYOUT = os.path.join(HERE, 'reservoir-temp-sensing.layout.json')
INTENT = os.path.join(HERE, 'reservoir-temp-sensing.intent.yaml')
...
# 边界端子：图幅边缘外部接口。本图只有回油边界 RET-001，坐标取自 layout.externs。
BOUNDARY_TERMINALS = [(120.0, 490.0)]
# 本图为新出图，无存量豁免；B2/B3 超限一律走 WARN，不带历史披露。
LEGACY_DISCLOSURE = '无存量豁免（本图非存量历史版本）'
...
def main():
    ...
    # 图例组内的演示三通点不是图面管件,按文本区间排除。
    lg_m = re.search(r'<g id="legend">.*?</g>\s*(?=<g id="title">)', raw, re.S)
    lg_span = (lg_m.start(), lg_m.end()) if lg_m else (-1, -1)
    jn = [(float(m.group(1)), float(m.group(2)))
          for m in re.finditer(
              r'<circle class="jn" cx="([\d.]+)" cy="([\d.]+)"', raw)
          if not (lg_span[0] <= m.start() <= lg_span[1])]
    ...
    # 本图适配:仅当 intent 确有吸油网络(端口 role=suction 的 path)时
    # 才要求斜杠组;无吸油网络的图(如本图)不适用本项。
    style = re.search(r'<style>(.*?)</style>', raw, re.S)
    srule = re.search(r'\.ln-suction\s*\{([^}]*)\}',
                      style.group(1) if style else '')
    suction_ports = {'suction'}
    suction_expected = False
    for p in intent['paths']:
        for tok in p:
            if tok.startswith('@') or '.' not in tok:
                continue
            inst, pid = tok.split('.', 1)
            ct = T.get(intent['parts'].get(inst))
            if ct and any(q['id'] == pid and q['role'] in suction_ports
                          for q in ct['ports']):
                suction_expected = True
    if not srule:
    ...
    for gi, g in enumerate(smgroups):
        ...
        smarks.extend(q)
    if suction_expected and not smgroups:
        F.append(('V17', '吸油线存在但未生成任何五斜杠组'))
    elif not suction_expected and smgroups:
        F.append(('V17', 'intent 无吸油网络却画了 %d 组斜杠' % len(smgroups)))
    ...
    LW = {'pressure': 3.0 * baseT, 'return': 1.0 * baseT,
          ...
          'sense': 1.0 * baseT}   # 感温/测量支路:1.0 T,不属压力等级
    # 用真正的解析器遍历实例,不用正则。
    # 早先版本用 <g id="inst-..">(.*?)\n</g> 抓实例体,非贪婪匹配在第一个
    ...
    # ---------- V4 三通实心点必须落在已画管线上 ----------
    # 本图无母线;三通点来自 taps 接入等场景,判据改为"点在某段管线上",
    # 比"在母线 x 上"更本质:三通点表达的就是三条线的连通。
    bus_x = {b['x'] for b in L['buses'].values()}
    for (x, y) in jn:
        on_bus = round(x, 1) in {round(v, 1) for v in bus_x}
        on_seg = any(
            abs(a[1] - b[1]) < 0.6 and abs(a[1] - y) < 0.6
            and min(a[0], b[0]) - 0.5 <= x <= max(a[0], b[0]) + 0.5
            or abs(a[0] - b[0]) < 0.6 and abs(a[0] - x) < 0.6
            and min(a[1], b[1]) - 0.5 <= y <= max(a[1], b[1]) + 0.5
            for _c, pts in polys for a, b in zip(pts, pts[1:]))
        if not (on_bus or on_seg):
            W.append(('V4', '三通点 (%g,%g) 不在任何已画管线上' % (x, y)))
    ...
    # taps 的 sensor measurement 端口按规范 11.1 挂入网络,不算悬空。
    for t in intent.get('taps') or []:
        tok = t['sensor']
        inst, pid = tok.split('.', 1) if '.' in tok else (tok, None)
        if pid is None:
            ct = T[intent['parts'][inst]]
            meas = [q['id'] for q in ct['ports'] if q['role'] == 'measurement']
            if len(meas) == 1:
                used.add((inst, meas[0]))
        else:
            used.add((inst, pid))
    dang = []
