...
import sys as _sys
_argv = _sys.argv[1:]
_base = _argv[0] if _argv else '1#系统'
SHEET = os.path.join(HERE, _argv[1] if len(_argv) > 1 else _base + '原理图.svg')
LAYOUT = os.path.join(HERE, _base + '.layout.json')
INTENT = os.path.join(HERE, _base + '.intent.yaml')
...
def main():
    ...
    out = os.path.join(HERE, (_base + '-validation-report.json') if _argv else 'validation-report.json')
