...
class Sheet(object):
    ...
    def wire(self):
        ...
        for a, b, pi, path_k, path_nseg, path_lt in nets:
            ...
            if ab and bb:
                # 总线-总线汇流(如用户回油收集母线汇入回油母线):布局
                # bus_links 显式给折线,确定性优先,不做自动布线。
                link = None
                for lk in self.L.get('bus_links') or []:
                    if {lk.get('from'), lk.get('to')} == {a[1:], b[1:]}:
                        link = lk
                        break
                if link is None:
                    self.warn.append('总线直连总线,未声明 bus_links: %s->%s' % (a, b))
                    continue
                pts = self.dedup([tuple(float(v) for v in q) for q in link['pts']])
                if len(pts) < 2:
                    self.warn.append('bus_links 折线不足两点: %s->%s' % (a, b))
                    continue
                lt = link.get('line_type', 'pressure')
                for q in range(len(pts) - 1):
                    self.drawn.append((pts[q], pts[q + 1]))
                self.polys.append((lt, pts))
                bus_hits.setdefault(a[1:], []).append((pts[0][1], lt))
                bus_hits.setdefault(b[1:], []).append((pts[-1][1], lt))
                # from 侧:连接段自母线端部延续,又有一条支路在此接入母线,
                # 图上是三通,必须打点;to 侧是母线端部拐角,不打。
                junctions.append((pts[0][0], pts[0][1]))
                continue
            if ab or bb:
...
BUDGET_DISCLOSURE = (
    'B1 交叉 0、B2 折返单条 3/全图 3、B4/B5/B6/B7 达标;'
    'B3 油箱回油线(@RET->TANK.return_in,顶绕走廊 y=100)绕行比 2.373 > 1.5 '
    '走 WARN 通道——根因是油箱单一 return_in 端口'
    '(unknown: TANK-001-return-port-count-unconfirmed),确认多回油口后本线可拆直。'
...
def write_manifest(path, intent, layout, s, path_polys, tap_polys, dnames):
    ...
    for pi, p in enumerate(intent['paths']):
        ...
        for k in range(len(p) - 1):
            ...
            if a_bus and b_bus:
                # 汇流连接:端点取 bus_links 声明的折线实端,不是占位 (x,0)。
                lk = next((l for l in layout.get('bus_links') or []
                           if {l.get('from'), l.get('to')} == {a[1:], b[1:]}), None)
                if lk:
                    qa = (float(lk['pts'][0][0]), float(lk['pts'][0][1]))
                    qb = (float(lk['pts'][-1][0]), float(lk['pts'][-1][1]))
                else:
                    qa, qb = bus_point(layout, a), bus_point(layout, b)
            else:
    ...
    L.append('2. 用户按 skill 更新后的通用用户框规范(hydraulic_user)绘制:'
             ...
             '分配母线接自优先阀 PRV-001,回油经 @USERR 收集母线于 (940,1000) '
             '汇入 @RET——两条母线是并联用户的绘图抽象;用户内部作动器/马达不在'
             '本图建模(concept,unknown: hydraulic-user-symbol-provisional)。')
